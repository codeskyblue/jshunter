...
def getOmitedFiles(confpath,topdir):
    ...
    if (not os.path.isdir(topdir)) and (len(_opt) > 0):
        return []
    for item in _opt:
        cmd = 'find %s/ -ipath "%s"'%(topdir,item)
        #print "~~~~~~~~~~~~~~"
        #print cmd
        #print "~~~~~~~~~~~~~~"
        (status,output) = commands.getstatusoutput(cmd)
...
def doJsHint(_path):
    ...
    if sz <= step:
        ...
        cmd = "%s -jar %s %s %s"%(javapath,jsjar,rhino,opt)
        print '---------', cmd
        (status,output) = commands.getstatusoutput(cmd)
    else:
        rd=sz/step+1
...
def getfiles(path, allowext = [], omitlist = []): 
    '''
       eg:
            allowext = ['.html', '.js']
            omitlist = [r'svn.*', r'.*god']
        program will change item in omitlist to 
            item = '^' + item + '$'
    '''
    files = []
    if os.path.isfile(path):
        try:
            ...
            not re.match('^\.', os.path.basename(path)): # not include hidden file
        try:
            for item in os.listdir(path):
                itemsrc = os.path.join(path, item)
                files += getfiles(itemsrc, allowext, omitlist)
        except:
            pass
    return files
...
def shellexec(filename):
    ...
    cmd = 'jre/bin/java -jar core/jshint/js.jar core/jshint/jshint-rhino.js %s %s'
    status,output = commands.getstatusoutput(cmd %('', filename))
...
if __name__ == "__main__":
    ...
    try:
        ...
        # parse command input
        outfile  = sys.argv[1]
        ignfile  = os.path.join(dirname(__file__), 'conf', 'ignore.list')
        chkfile  = os.path.join(dirname(__file__), "conf", "check.cfg")
        omitfile = os.path.join(dirname(__file__), 'conf', 'omitfiles.conf')

        print ignfile
        #print open(ignfile).read()
        #print open(omitfile).read()
        #blackpath = os.path.dirname(__file__) + os.sep + "conf" + os.sep + "ignore.list"
        #filepath = sys.argv[2:]

        
        files = getfiles(sys.argv[2], ['.js'], omitlist = [])
        print files
        hintoutput = ""
        for f in files:
            cmd = 'jre/bin/java -jar core/jshint/js.jar core/jshint/jshint-rhino.js %s %s' \
        ...
        #opt = "%s %s"%(getopt(chkfile), 'hello') #" ".join(fileToCheck[:]))
        #print opt
        ...
        # generate report
        if hintoutput != "":
            print "[FILE TO REPORT ]%s"%(outfile) 
            ts = genReport(0, hintoutput, ignfile, outfile, htmlMap)
        else:
